...
function ajaxAddBook(form) {
    ...
    axios(addBook).then(res => {
        if(res.data.code!=0){
            ...
            alertLayer("编辑书籍", res.data.msg, closeForm)
        };
    }).catch(err => {
        alertLayer2("编辑书籍", err);
    })
}
