...
//layer的問題，需要改變zindex並且在調用弹出窗口時（登錄和退出時）改變z-index
...
function pjChapters(allChapterMenu) {
    ...
    if(!allChapterMenu||allChapterMenu.length==0){
        ...
        for(let i=0;i<allChapterMenu.length;i++){
            ...
            let id = item.id;
            nHtml+='<li onclick="readBook2('+id+')" class="chapter-li">第'+item.sort+'章' +
                '<span style="margin-left: 25%">'+item.title+'</span></li>'
        }
    }
    return nHtml;
}
...
//弹出窗口模板2  带时间laydate的弹窗  html位js拼接的数据
